...
for f in f_c:
    ...
    for time in time_set:    
        for num in num_set:
            file = "results/Corr/ang_origin/D_" + str(int(num)) + "/F5_" + str(int(time)) + "_r_2.5_f_" + str(f_c[counter]) + ".npy"
            p_hop = np.load("results/Corr/data/D_" + str(int(num)) + "/P_hop_" + str(int(time)) + ".npy", allow_pickle=True)
            X = np.load(file, allow_pickle=True)
            sh = np.load("results/Corr/data/D_" + str(int(num)) + "/Y_" + str(int(time)) + ".npy", allow_pickle=True)
            x0 = X[:, 0]
            x1 = X[:, 1]
            x2 = X[:, 2]
            x3 = X[:, 3]
