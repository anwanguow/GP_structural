                ...
                for j in node:
                    ...
                    for k in node:
                        if (i != j) and (j != k) and (i != k):
                            ...
                            s = s + s_
                cost[i,ft] = s
        cost = normalize(cost, axis=0, norm='max')      
        np.save("data/D_" + str(int(p)) + "/D_" + str(int(t)) + "_r_" + str(r_c) + "_f_" + str(f_c) + ".npy", cost, allow_pickle=True)
